...
pub struct MemPortalStore<S, PS> {
    #[new(default)]
    statements: RwLock<BTreeMap<String, Arc<StoredStatement<S>>>>,
    #[new(default)]
    portals: RwLock<BTreeMap<String, Arc<Portal<S, PS>>>>,
}

impl<S: Clone + Send + Sync, PS: Clone + Send + Sync> PortalStore for MemPortalStore<S, PS> {
    type Statement = S;
    type State = PS;
